...
+function($) {
	...
	Wizard.prototype.saveStep = function (fnSuccess) {
		...
		var nextOptions  = JSEA.Jsons.parse($nextTrigger.attr(JSEA.Constants.ATTR_BUTTON_OPTIONS));
		this.$form.data('jsea.plugin').addPostHandler('next', fnSuccess).perform({
			name     : 'next',
			mode     : 'save',
			url      : nextOptions.url,
			success  : nextOptions.success || false
		});
	};
